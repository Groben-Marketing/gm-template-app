...
## [Unreleased]
### Added
- **The TopNav "Report a Bug" button is now a working, reusable bug reporter** (`@wayfinder/bug-reporter`, r7c app-clients epic). Previously `onBugReport` was a placeholder `alert()`; it now opens a `BugModal` that captures the current route/build/browser and POSTs to `/api/bug-report`. The server (`mountBugReporter`) HMAC-signs the report and forwards it to Workbench, where it becomes a ticket in the same lifecycle/portal/emails as every other request — so **every app cloned from the template inherits bug reporting for free**. No secret in the browser; the endpoint no-ops safely until `WORKBENCH_BUG_WEBHOOK_URL` + `WORKBENCH_BUG_SECRET` + `GITHUB_REPO` are set. New dep `@wayfinder/bug-reporter` (git, same convention as `@wayfinder/email-sender`); new `src/components/BugModal.tsx`.
- **The app's structure is real from clone — `server/` and `supabase/` now ship in the template.** Previously the Hono server lived in a root `starter-server.ts` you had to *copy* to `server/index.ts` (which `npm run dev`/`start` and the Dockerfile already expected), and `supabase/` didn't exist at all — so a fresh clone couldn't run and *looked* unbuilt. Now `server/index.ts` exists and `supabase/migrations/0001_init.sql` (`profiles` + example `items`, RLS on both) + `supabase/README.md` ship. Closes the gap where "is the app built?" wasn't answerable from repo structure.
- **Guardrail: verify the real app, never a wireframe/mockup** (`docs/agent-verification.md` + `.claude/commands/qa.md`, both synced). New standard section names the exact failure mode that certifies decoration as a working feature: a repo often *also* holds Claude Design exports (`*.dc.html`), `docs/prototype/`, `docs/mockups/` — pictures of the app, not the app. `/qa` now requires confirming you're driving the running app (Vite/Hono-served URL, a real action round-tripped to `/api/*`, `server/`+`supabase/` exist) before walking anything; if the repo is only docs + design exports, say the app isn't built and stop. `README-TEMPLATE.md` states the same "app is `src/`+`server/`+`supabase/`" rule app-facing.
- **`docs/auth-performance-standard.md`** — the Auth-Performance Standard, distilled from `gm-app-prospect-factory` #122 (which had a 1–7s-per-page floor that was never the DB: the query was 0.29ms). Five enforceable rules: (1) **verify the JWT locally with `auth.getClaims()`, never `auth.getUser()` per request** — `getUser()` is a network round-trip to the Supabase Auth server on *every* request (the ~1–2s floor); `getClaims()` verifies the ES256 signature against a cached JWKS locally. (2) Parallelize + short-TTL-cache the remaining middleware reads (memberships/profile/admin); RLS stays the hard boundary. (3) Hydrate the SPA once per access token (kill the `INITIAL_SESSION`+`SIGNED_IN`+`getSession()` triple-fire). (4) Stale-while-revalidate client cache for read-heavy navigation. (5) Paginate heavy table scans / drop embedded joins from full scans. Measured wins on the reference app: `/api/me` 1974–7375ms→~300ms, bootstrap 3×→1×, a 774KB/1.4s list route →154KB/0.55s. Synced downstream.
...
### Changed
- **`starter-server.ts` → `server/index.ts`** — the Hono server moved into the `server/` dir it always belonged in (via `git mv`, history preserved). Stale references updated across `docs/fault-isolation-standard.md` and `docs/migration-checklist.md` (the "copy `starter-server.ts`" step is gone). `scripts/compliance-scan.sh` already tolerated both paths.
- **`server/index.ts` auth middleware now verifies the JWT locally** (`auth.getClaims()` instead of `auth.getUser()`) and caches the per-user profile/role read for 20s — implements rules 1–2 of `docs/auth-performance-standard.md` so new apps inherit the fast path by default. Behaviour-preserving (still rejects expired/tampered tokens; same `user`/`profile`/`isAdmin` context).
- **Sync pipeline repaired + hardened** (`.github/workflows/sync-to-repos.yml`, `.github/sync-config.json`) — the downstream sync had been a false green: when `REPO_SYNC_TOKEN` fails, every clone was swallowed by `|| continue` and the job still exited 0, so nothing propagated while the run showed success. Now: the job **fails loudly if every clone fails** (broken-token detection); the sync branch is **unique per run** (`…-${GITHUB_RUN_ID}`) instead of per-day, so multiple same-day pushes no longer collide and silently skip; the four v1.7.0 standard docs + `scripts/compliance-scan.sh` are added to the `paths:` trigger (a future edit to a standard alone now re-fires the sync); the PR-body link points at the real template repo; and archived `Wayfinder-Digital/yg-app-loopback` is removed from the sync targets.
### Fixed
### Removed
