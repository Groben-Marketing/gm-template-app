...
## [Unreleased]
### Added
- **`docs/discovery-principles.md`** — template-level doctrine: how to discover and spec an operations app. Product thesis (cognitive-load reduction for complex operations), the three delivery mechanisms (objective/subjective split, role-based cognitive scoping, focus queue), 10 discovery principles, the discovery method in order, and anti-patterns. Imported verbatim from `Wayfinder-Digital/jnm-app-ops-os@ed1fb30` (header note adjusted from "intended for promotion into the template" to "canonical template copy"; body unchanged). Registered for propagation in `.github/sync-config.json`.
- **`docs/cognitive-scoping-doctrine.md`** — template-level doctrine: role-based cognitive scoping ("each role sees only their slice"). Scoping vs. security/RLS, how to scope a role (four questions → view spec), the required admin "view-as" toggle, and acceptance checks. Same source and provenance as above. Registered for propagation in `.github/sync-config.json`.
...
### Changed
- **`.github/sync-config.json`**: Added `docs/discovery-principles.md` and `docs/cognitive-scoping-doctrine.md` to the propagation list.
- **`.github/workflows/sync-to-repos.yml`**: Added both new doctrine files to the push-path triggers, and added `docs/r7c-standards.md`, which was already in `sync-config.json` but missing from the trigger list (pre-existing drift — edits to it would not have fired the sync).
- **`docs/r7c-standards.md`**: New *Template-Level Doctrine* section indexing both doctrine files and noting they are level-2 template doctrine (propagated like `architecture-patterns.md`) without a backing `r7c-context/standards/` document yet; reading-order note added for speccing/role-view work.
- **`docs/architecture-patterns.md`**: New *Role Views & Cognitive Scoping* section cross-linking the doctrine files and stating that cognitive scoping never substitutes for RLS.
- **`REPO-STRUCTURE-GUIDE.md`**: Both doctrine files added to the `docs/` tree and the Quick Reference table ("Spec a new operations app", "Reduce a role's view to what it needs to see"); *Role-Based Access* section now points to the cognitive-scoping doctrine.
